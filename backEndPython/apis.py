...
def allExpiryDates():
    try:
        ...
        # Convert dates to datetime objects and filter
        filtered_dates = [date for date in data if datetime.strptime(date, '%Y-%m-%d').date() >= current_date]
    except:
        data = "Error"
    response = {"data": filtered_dates}
    return jsonify(response)
...
def OIData():
    try:
        ...
        df['PCR'] = np.where(df["S_OI_Calls"] != 0, df['S_OI_Puts'] / df["S_OI_Calls"], 0).round(2)
        ...
        df=df.tail(1)
        df = df[['Date', 'S_OI_Calls', 'S_OI_Puts', 'PCR']]
